  ...
  return (
    <ToastProvider>
      <div className="min-h-screen bg-[#dfff00] text-black font-mono font-bold selection:bg-black selection:text-[#dfff00]">
        ...
        <nav className="fixed top-6 left-1/2 -translate-x-1/2 w-fit max-w-[95vw] z-50 border-4 border-black rounded-full bg-[#dfff00]/20 backdrop-blur-[1px] flex justify-between items-center px-6 py-3 md:px-14 md:py-5 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] gap-3 sm:gap-10 md:gap-16 whitespace-nowrap">
          <a href="#gallery" className="text-base sm:text-3xl md:text-5xl uppercase hover:glitch-text transition-all font-mono font-black shrink hover:text-[#ff00ff]">Gallery</a>
          <a href="#events" className="text-base sm:text-3xl md:text-5xl uppercase hover:glitch-text transition-all font-mono font-black shrink hover:text-[#ff00ff]">Shows</a>
          ...
          <a
            ...
            className="mx-3 md:mx-10 select-none flex items-center justify-center flex-shrink-0 min-w-fit hover:scale-110 active:scale-95 transition-transform"
          >
            {siteSettings.site_logo_url ? (
              <img src={siteSettings.site_logo_url} alt="Logo" className="h-14 sm:h-24 md:h-36 w-auto object-contain drop-shadow-[5px_5px_0px_rgba(0,0,0,1)] flex-shrink-0" />
            ) : (
              <span className="text-3xl sm:text-6xl md:text-8xl font-logo leading-none text-center flex flex-col uppercase flex-shrink-0">
          ...
          <a href="#store" className="text-base sm:text-3xl md:text-5xl uppercase hover:glitch-text transition-all font-mono font-black shrink hover:text-[#ff00ff]">Store</a>

          <button
            onClick={handleMenuClick}
            className="relative w-8 h-5 sm:w-12 sm:h-6 z-50 flex-shrink-0 cursor-pointer group flex flex-col justify-around items-center"
            aria-label="Menu"
          >
            <motion.span
              animate={isMenuOpen ? { rotate: 45, y: 6 } : { rotate: 0, y: 0 }}
              className="w-full h-[4px] bg-black block transition-all group-hover:bg-[#ff00ff]"
            />
            <motion.span
              animate={isMenuOpen ? { rotate: -45, y: -6 } : { rotate: 0, y: 0 }}
              className="w-full h-[4px] bg-black block transition-all group-hover:bg-[#ff00ff]"
